...
class ShopistApp: XCUIApplication {
    @discardableResult
    func launchToHomepage(with scenario: Scenario) -> CategoriesPage {
        guard let encodedScenario = try? JSONEncoder().encode(scenario),
              let encodedScenarioString = String(data: encodedScenario, encoding: .utf8) else {
            fatalError("Non-encodable scenario!\n\(scenario)")
        }
        launchEnvironment[Scenario.envVarKey] = encodedScenarioString

        // Propagate ENV variables to Shopist app process
        ProcessInfo.processInfo.environment.forEach { variable, value in
            launchEnvironment[variable] = value
        }
